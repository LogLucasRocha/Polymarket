...
\begin{itemize}
  ...
  \item \textbf{Alerta de compra novo:} vai enxuto, com um botão
  ``Ver relatório completo''. O bloco pesado (tabela, gráfico, hora a hora) só
  é enviado sob demanda (botão ou \texttt{/relatorio}), para não poluir o chat.
  \item \textbf{Repetição:} \emph{edge} e colheita repetem a cada rodada
  ($\sim$10\,min) enquanto a oportunidade durar --- uma entrada equivale a
  muitas mensagens.
